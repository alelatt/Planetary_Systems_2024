...
plt.figure(dpi = 100, layout = 'tight')
plt.plot(wavelength*1e9, 100*(binned_1989-binned_2019)/binned_1989, 'k')
plt.xlabel("Wavelength [nm]")
plt.ylabel(r"$\Delta F_{\lambda}$/F$_{\lambda, 1989}$ [%]")
plt.grid()
#plt.show()
...
plt.xlabel("Wavelength [nm]")
plt.ylabel(r"$\sigma_{\lambda}^{diss}$ [cm$^2$]")
plt.yscale('log')
plt.grid()
#plt.show()
...
plt.yscale('log')
fig2.text(0.22, 0.9, f"Total H produced:\n  1989: {np.sum(number_1989)*M_H*1e-9:.1f} kg/s\n  2019: {np.sum(number_2019)*M_H*1e-9:.1f} kg/s", ha="left", va="top", bbox=dict(boxstyle='round', fc='blanchedalmond', ec='orange', alpha=1))
plt.legend(loc = "lower right")
plt.grid()
plt.show()
